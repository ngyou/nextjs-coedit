...
export default function AdminDocumentHistoryPage() {
  ...
  const withAuth = (res: Response): boolean => {
    if (res.status === 401) {
      setAdminToken("");
  ...
  const loadData = async (nextOffset: number) => {
    ...
    try {
      ...
      setSelectedId(snapshotData.items[0]?.id ?? null);
    } catch (e) {
  ...
  const downloadSnapshot = async (snapshotId: number) => {
    setError("");
    try {
      const res = await adminFetch(`/api/admin/documents/${encodeURIComponent(docId)}/snapshots/${snapshotId}`);
      if (!withAuth(res)) return;
      if (!res.ok) throw new Error("Failed downloading snapshot");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `${docId}-snapshot-${snapshotId}.bin`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed downloading snapshot");
    }
  };
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-5 px-6 py-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600">Doc ID: <span className="font-mono">{docId}</span></p>
        </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <p className="text-xs text-slate-500">Showing {snapshots.length} rows at offset {offset}</p>
        </div>

        <div className="grid gap-4 lg:grid-cols-2">
          <article className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <h2 className="mb-2 text-sm font-semibold text-slate-900">Char Count Trend</h2>
            <p className="mb-2 text-xs text-slate-500">Min {charChart.min} / Max {charChart.max}</p>
          ...
          <article className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            ...
            <p className="mb-2 text-xs text-slate-500">Min {formatBytes(sizeChart.min)} / Max {formatBytes(sizeChart.max)}</p>
            <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-56 w-full rounded-md bg-white">
              {sizeChart.polyline ? (
        ...
        {selected ? (
          <aside className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
            ...
            <p>Snapshot ID: <span className="font-mono">{selected.id}</span></p>
